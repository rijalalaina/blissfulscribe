...
class HistoryWindowController: NSObject, NSWindowDelegate {
    ...
    private let windowIdentifier = NSUserInterfaceItemIdentifier("com.goodtogreatmind.blissfulscribe.historyWindow")
    private let windowAutosaveName = NSWindow.FrameAutosaveName("BlissfulScribeHistoryWindowFrame")

    private override init() {
        super.init()
    }

    func showHistoryWindow(modelContainer: ModelContainer, engine: BlissfulScribeEngine) {
    ...
    private func createHistoryWindow(modelContainer: ModelContainer, engine: BlissfulScribeEngine) -> NSWindow {
