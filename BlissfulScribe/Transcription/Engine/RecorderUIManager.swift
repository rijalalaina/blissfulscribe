...
class RecorderUIManager: ObservableObject, RecorderPanelPresenting {
    ...
    private weak var engine: BlissfulScribeEngine?
    private var recorder: Recorder?

    private let logger = Logger(subsystem: "com.goodtogreatmind.blissfulscribe", category: "RecorderUIManager")

    init() {}

    /// Call after BlissfulScribeEngine is created to break the circular init dependency.
    func configure(engine: BlissfulScribeEngine, recorder: Recorder) {
        self.engine = engine
